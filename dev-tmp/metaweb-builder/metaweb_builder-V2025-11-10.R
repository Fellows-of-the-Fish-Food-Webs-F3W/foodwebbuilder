...
for (j in 2:(ncol(tab_size_classes)-1))
{
  ...
  tab_size_classes_midpoints = cbind(tab_size_classes_midpoints, tab_size_classes_midpoints_)
}
...
## Vectorise 
vec_size_classes_midpoints = as.numeric(t(tab_size_classes_midpoints))
vec_lb_prey = as.numeric(t(lb_prey))
vec_ub_prey = as.numeric(t(ub_prey))

## Build fish-fish interaction matrix
ff_interactions = NULL
for (i in 1:length(vec_size_classes_midpoints)){
  ff_interactions_ = (vec_size_classes_midpoints >= vec_lb_prey[i]) * (vec_size_classes_midpoints < vec_ub_prey[i])  
...
for (i in 1:length(trophic_species_code)){
  ...
  ## Find match
  check = (vec_size_classes_midpoints[i] >= fish_diet_shift_$size_min) & (vec_size_classes_midpoints[i] < fish_diet_shift_$size_max)
