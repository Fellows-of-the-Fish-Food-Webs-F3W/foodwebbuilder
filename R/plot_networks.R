...
plot_network = function(M, x=NULL, y=NULL, labels=NULL, xlab="", ylab="", line_width_max=1, label_space_x=0.05, add_legend="topright"){
  ...
  for(j in 1:ncol(M))
  {
    color_ = grDevices::rainbow(ncol(M))[j]
    for(i in 1:nrow(M))
    {
      line_width = abs(M[i,j])/max(abs(M)) * line_width_max
      graphics::lines(x=c(x[i],x[j]-delta_x/2), y=c(y[i],y[j]), col=color_, lwd=line_width)
      graphics::arrows(x0=x[i], x1=x[j]-delta_x/2, y0=y[i], y1=y[j], col=color_, lwd=line_width)
    }
  }
  ...
  if (add_legend != "off"){
    graphics::legend(add_legend, legend=c("Ingoing effects","Outgoing effects"), pch=c(1,16), col=c("black","black"), cex=1.5, bg="white", box.col="white")
  }
...
plot_network_radial = function(M, x=NULL, y=NULL, labels=NULL, line_width_max=1, rotations=1.0, scale=1.0, x_offset=0.0, y_offset=0.0, add_legend="topright"){
  ...
  for(j in 1:d)
  {
    color_ = grDevices::rainbow(d)[j]
    graphics::text(x__[j],y__[j], labels=labels[j])
    for(i in 1:d)
    {
      line_width = abs(M[i,j])/max(abs(M))*line_width_max
      graphics::lines(x=c(x[i],x_[j]), y=c(y[i],y_[j]), col=color_, lwd=line_width)
      if (((x[i] - x_[i])^2 + (y[i] - y_[j])^2) > 0) graphics::arrows(x0=x[i], x1=x_[j], y0=y[i], y1=y_[j], col=color_, lwd=line_width)
    }
  }
  ...
  if (add_legend != "off"){
    graphics::legend("bottomright", legend=c("Ingoing effects","Outgoing effects"), pch=c(1,16), col=c("black","black"), cex=1.5, bg="white", box.col="white")
  }
}
